# M16 – Step 2: Offline-First & Sync Design
...
5. Background sync loop detects network restored → replays queue
...
- **CASH payments**: Can be taken offline
  ...
  - Replay when online → server posts GL entries
...
3. Offline modification replays → server sees newer `updatedAt` → rejects with 409 Conflict
...
async function syncKdsTickets(station: string) {
  const response = await fetch(`/kds/queue?station=${station}&since=${lastSyncAt.toISOString()}`);
...
- Guest bookings involve payment intents (Flutterwave/MTN MoMo) → requires online payment gateway
- Deposit capture requires real-time GL posting → requires database write
...
- ✅ Same key for retrying "create order #123"
- ❌ Same key for "create order #123" and "create order #456"
...
**Optional Enhancements**: 6. **Background Sync API**: Use Service Worker for automatic replay when browser closed 7. **Optimistic UI**: Show operation success immediately, rollback if replay fails
...
**Nice-to-Have**: 5. **WebSocket Fallback**: For real-time ticket updates (avoids polling)
...
```
┌─────────────────────────────────────┐
│ Order #1234  Table 5      $25,000  │
│ ✅ Synced                           │
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│ Order #1235  Table 8      $18,000  │
│ 🔄 Syncing... (2 operations pending)│
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│ Order #1236  Table 3      $42,000  │
...
```
┌─────────────────────────────────────────┐
│ 🟢 Connected - Last sync: 3 seconds ago │
└─────────────────────────────────────────┘

┌─────────────────────────────────────────┐
│ 🟡 Reconnecting... (2 updates queued)   │
└─────────────────────────────────────────┘
...
- Example: Waiter A adds item X, Waiter B adds item Y → both items appear
