...
describe('Transfer Invalidation (Slice E2E) — E22.D', () => {
  ...
  it('HIT → transfer.changed → MISS (forecast cache proves invalidation)', async () => {
    ...
    const r1 = await request(srv).get('/forecast-test/sales?period=2025-11').set(AUTH).ok(() => true);
    if (r1.status === 429) return; // Skip if throttled
    const v1 = r1.body?.version;
    const r2 = await request(srv).get('/forecast-test/sales?period=2025-11').set(AUTH).ok(() => true);
    if (r2.status === 429) return; // Skip if throttled
    expect(r2.headers['x-cache']).toBe('HIT');
    expect(r2.body?.version).toBe(v1);

    // Fire transfer.changed
    const evt = { id: 'evt-x1', type: 'transfer.changed', data: { from: 'A', to: 'B', sku: 'SKU-1', qty: 4, at: new Date().toISOString() } };
    ...
    const r3 = await request(srv).get('/forecast-test/sales?period=2025-11').set(AUTH).ok(() => true);
  ...
  it('Idempotency: repeating same event still returns ok:true', async () => {
    const srv = app.getHttpServer();
    const evt = { id: 'evt-repeat', type: 'transfer.changed', data: { from: 'A', to: 'B', sku: 'SKU-2', qty: 1, at: new Date().toISOString() } };
  ...
  it('Deterministic rate limit: >= one 429 on /forecast-test/sales', async () => {
    ...
    const codes: number[] = [];
    for (let i = 0; i < 7; i++) {
      const r = await request(server).get('/forecast-test/sales?period=2025-12').set(AUTH).ok(() => true);
