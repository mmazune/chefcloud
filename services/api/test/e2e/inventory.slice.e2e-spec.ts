...
describe('Inventory (Slice E2E) — Deterministic', () => {
  ...
  describe('Authentication & Authorization', () => {
    it('GET /inventory/items should return 401 if no authorization token is provided', async () => {
    ...
    it('POST /inventory/items should return 401 if no authorization token is provided', async () => {
      const res = await request(app.getHttpServer())
        .post('/inventory/items')
        .send({ sku: 'TEST-SKU', name: 'Test Item', uom: 'kg' });

      expect(res.status).toBe(401);
    });

    it('POST /inventory/adjustments should return 401 if no authorization token is provided', async () => {
      const res = await request(app.getHttpServer())
        .post('/inventory/adjustments')
        .send({ itemId: 'item_001', deltaQty: -5, reason: 'Test' });

      expect(res.status).toBe(401);
    });

    it('POST /inventory/wastage should return 401 if no authorization token is provided', async () => {
      const res = await request(app.getHttpServer())
        .post('/inventory/wastage')
        .send({ itemId: 'item_001', quantity: 1, reason: 'Spoiled' });
  ...
  describe('Basic Functionality (without auth)', () => {
    ...
    it('GET /inventory/items should exist (not 404)', async () => {
      const res = await request(app.getHttpServer())
        .get('/inventory/items');
    ...
    it('POST /inventory/adjustments should validate request body', async () => {
      const res = await request(app.getHttpServer())
        .post('/inventory/adjustments')
        .send({}); // Missing required fields
  ...
  describe('Endpoint Availability', () => {
    it('GET /inventory/items endpoint exists', async () => {
      const res = await request(app.getHttpServer())
        .get('/inventory/items');
    ...
    it('POST /inventory/items endpoint exists', async () => {
      const res = await request(app.getHttpServer())
        .post('/inventory/items')
        .send({ sku: 'TEST', name: 'Test', uom: 'kg' });

      expect(res.status).not.toBe(404);
    });

    it('GET /inventory/levels endpoint exists', async () => {
      const res = await request(app.getHttpServer())
    ...
    it('POST /inventory/adjustments endpoint exists', async () => {
      const res = await request(app.getHttpServer())
        .post('/inventory/adjustments')
        .send({ itemId: 'item_001', deltaQty: -5, reason: 'Test' });
    ...
    it('POST /inventory/wastage endpoint exists', async () => {
      const res = await request(app.getHttpServer())
        .post('/inventory/wastage')
        .send({ itemId: 'item_001', quantity: 1, reason: 'Test' });
