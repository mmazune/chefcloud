...
export class CsvGeneratorService {
  ...
  generateSalesCSV(report: ShiftEndReport): string {
    ...
    report.sales.byItem.forEach((item) => {
      ...
      lines.push(
        `${this.escapeCSV(category)},${this.escapeCSV(item.itemName)},${item.quantity},${item.revenue},${((item.revenue / report.sales.totals.revenue) * 100).toFixed(2)}`,
      );
    });
  ...
  generateServiceCSV(report: ShiftEndReport): string {
    ...
    // Header
    lines.push(
      'Waiter,Orders,Revenue,Avg Check,Voids,Void Amount,Discounts,Discount Amount,No-Drinks Rate',
    );
  ...
  generateShiftEndCSV(report: ShiftEndReport): string {
    ...
    sections.push(
      `Period: ${report.period.startedAt.toISOString()} to ${report.period.closedAt.toISOString()}`,
    );
    sections.push('');
