...
export interface BranchMetrics {
  ...
  // Customer Satisfaction (from Feedback M20)
...
export class FranchiseOverviewService {
  ...
  async getBranchMetrics(
    ...
    this.logger.log(
      `Fetching branch metrics for ${branchId}, period ${periodStart.toISOString()} - ${periodEnd.toISOString()}`,
    );
    ...
    try {
      ...
      if (feedbackData.length > 0) {
        ...
        nps = Math.round(avgScore * 10) / 10;
      }
    } catch (err) {
      this.logger.warn(`Failed to get NPS for branch ${branchId}: ${err}`);
    }

    // 7. Budget vs Actual (if configured)
  ...
  async getFranchiseSummary(
    ...
    this.logger.log(
      `Fetching franchise summary for org ${orgId}, period ${periodStart.toISOString()} - ${periodEnd.toISOString()}`,
    );
    ...
    const branchMetrics = await Promise.all(
      branches.map((branch) => this.getBranchMetrics(orgId, branch.id, periodStart, periodEnd)),
    );
    ...
    const avgKdsSlaScore =
      branchMetrics.reduce((sum, b) => sum + b.kdsSlaScore, 0) / branchMetrics.length;
    const avgStaffScore =
      branchMetrics.reduce((sum, b) => sum + b.staffScore, 0) / branchMetrics.length;

    // Aggregate budget data
    const branchesWithTargets = branchMetrics.filter((b) => b.revenueTarget !== undefined);
    const totalRevenueTarget = branchesWithTargets.reduce(
      (sum, b) => sum + (b.revenueTarget || 0),
      0,
    );
    const totalRevenueDelta = branchesWithTargets.reduce(
      (sum, b) => sum + (b.revenueDelta || 0),
      0,
    );
