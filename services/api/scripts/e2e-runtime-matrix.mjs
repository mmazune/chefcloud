...
async function runTestFile(filePath, index, total) {
  ...
  console.log(
    `         Elapsed: ${Math.floor(elapsedTotal / 60000)}m ${Math.floor((elapsedTotal % 60000) / 1000)}s | Remaining: ${remainingMinutes}m ${remainingSeconds}s`,
  );
        ...
        try {
          ...
          if (jsonData.testResults && jsonData.testResults.length > 0) {
            ...
            if (firstResult.assertionResults) {
              ...
              if (
                firstFailure &&
                firstFailure.failureMessages &&
                firstFailure.failureMessages.length > 0
              ) {
