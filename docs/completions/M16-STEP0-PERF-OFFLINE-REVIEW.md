...
**Medium Priority**: 6. **Order** `(branchId, createdAt)` composite (shift-end aggregations) 7. **JournalEntry** `(accountId, postedAt)` (balance sheet queries) 8. **TimeEntry** `(employeeId, date)` (attendance reports)
