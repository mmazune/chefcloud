...
- ✅ Created migration for missing `users.jobRole` column
- ✅ Fixed FK cleanup order (stock_movements before inventoryItem)
- ✅ Added seed verification script with dataset integrity checks
- ✅ Integrated verification into E2E setup (fail-fast on missing data)
- ✅ Updated matrix/CI runners to run setup before tests
- ✅ Verified DEMO_TAPAS dataset is seeded with 178 menu items, 11 users, 158 inventory items, 305 orders
...
await prisma.inventoryItem.deleteMany({}); // ❌ FAILED - FK constraint violation
await prisma.stockMovement.deleteMany({});

// After cleanup sequence:
await prisma.stockMovement.deleteMany({}); // ✅ Delete children first
...
npx tsx "$SEED_SCRIPT" || { echo "❌ Seed failed"; exit 1; }

# NEW: Verify seed data integrity
node "$VERIFY_SCRIPT" ALL || {
  echo "❌ Seed verification failed - dataset is incomplete"
  exit 1
}
```
...
async function main() {
  ...
  if (setupResult !== 0) {
    console.error('❌ E2E setup failed - cannot proceed');
    process.exit(1);
  }
...
```
✅ Org found: Cafesserie (00000000-0000-4000-8000-000000000002)
✅ Found 4 branches (franchise)
✅ Found 320 menu items
✅ Found 8 users
✅ Found orders across 4 branch(es)
✅ DEMO_CAFESSERIE_FRANCHISE verification passed
```
...
- ❌ Old assumption: `tapas-bar`, `demo-empty`, `cafesserie`
- ✅ Actual slugs: `tapas-demo`, `cafesserie-demo`
...
- ✅ DEMO_TAPAS exists (slug: `tapas-demo`)
- ✅ DEMO_CAFESSERIE_FRANCHISE exists (slug: `cafesserie-demo`)
- ❌ DEMO_EMPTY doesn't exist - only legacy `demo-restaurant` (11 users, 3 menu items)
...
✅ **Fixed schema drift** - Created migration for missing `jobRole` column  
✅ **Fixed FK cleanup** - Reordered stock_movements deletion before inventoryItem  
✅ **Added verification** - Seed integrity checks with fail-fast on incomplete data  
✅ **Updated runners** - Matrix and CI now run setup before tests  
✅ **Verified datasets** - DEMO_TAPAS and DEMO_CAFESSERIE_FRANCHISE fully seeded
