...
7. test/e2e/auth.e2e-spec.ts ✅ FIXED
...
- Connection retries 3 times, each retry causes delay, total ~120s → TIMEOUT
...
→ TIMED_OUT (hit 10m limit, killed)
```
...
→ PASS test/e2e/auth.e2e-spec.ts
  Auth E2E
    ✓ should login with email/password and return access_token (157 ms)
    ✓ should reject invalid password (121 ms)
