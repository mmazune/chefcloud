# T1.11 — E2E Runtime Matrix: COMPLETION REPORT
...
**Outcome:** ✅ **CONFIRMED (100%)** - 12 files timeout at exactly 120s
...
**Outcome:** ✅ **PARTIALLY CONFIRMED (70%)** - Bimodal distribution shows setup isn't the primary bottleneck, but still contributes
...
**Outcome:** ❌ **DENIED (20%)** - 12 different files timeout, not just SSE-related ones
...
**Outcome:** ❌ **DENIED (10%)** - Files either complete <10s or timeout (120s+); no gradual slowdown pattern
...
|---------|---------|--------|----------|
| `timeout 30s pnpm test:e2e -- --listTests` | 30s | ❌ Failed (Jest --listTests broken) | ~5s |
| `find test -name "*.e2e-spec.ts"` | N/A | ✅ Success (57 files found) | <1s |
| `timeout 30m node scripts/e2e-runtime-matrix.mjs` | 30m (25m internal) | ⏱️ **BUDGET EXHAUSTED** | **25m 15s** |
| `timeout 30s pnpm test:e2e -- billing.blackbox` | 30s | ✅ Success (2 passed) | ~2s |
...
|------|----------|--------|------|---------|
| 1 | 2m 0s | ⏰ **TIMED_OUT** | `test/e24-subscriptions.e2e-spec.ts` | **CRITICAL** |
| 2 | 2m 0s | ⏰ **TIMED_OUT** | `test/e27-costing.e2e-spec.ts` | **CRITICAL** |
| 3 | 2m 0s | ⏰ **TIMED_OUT** | `test/auth.e2e-spec.ts` | **CRITICAL** |
| 4 | 2m 0s | ⏰ **TIMED_OUT** | `test/a3-pos.e2e-spec.ts` | **CRITICAL** |
| 5 | 2m 0s | ⏰ **TIMED_OUT** | `test/e26-kpis.e2e-spec.ts` | **CRITICAL** |
| 6 | 2m 0s | ⏰ **TIMED_OUT** | `test/e2e/badge-revocation.e2e-spec.ts` | **CRITICAL** |
| 7 | 2m 0s | ⏰ **TIMED_OUT** | `test/e22-franchise.e2e-spec.ts` | **CRITICAL** |
| 8 | 2m 0s | ⏰ **TIMED_OUT** | `test/e2e/franchise.slice.e2e-spec.ts` | **CRITICAL** |
| 9 | 2m 0s | ⏰ **TIMED_OUT** | `test/e2e/devportal.slice.e2e-spec.ts` | **CRITICAL** |
| 10 | 2m 0s | ⏰ **TIMED_OUT** | `test/e2e/auth.e2e-spec.ts` | **CRITICAL** |
...
**Why this file:**
- Core POS functionality (order creation → KDS → payment)
...
### **Immediate Action Plan (T1.11a — NO CODE YET, separate task)**
...
✅ **Dataset: DEMO_TAPAS** (defaulted automatically per DEMO_TENANTS_AND_DATASETS.md)

**Evidence:**
- Matrix runner logged: `🗂️  Dataset: DEMO_TAPAS`
...
### **Root Cause of 25m Timeout: IDENTIFIED ✅**

**The Problem:**
- **12 out of 57 test files (21%) never complete**
- Each times out at exactly 120s
- Running all 57 files sequentially would take **114 minutes minimum** (12 × 2m + 45 × ~5s)
...
**Current state:**
- 12 timeout files × 120s = **1440s (24 minutes)** wasted on timeouts alone
- 17 healthy files × ~5s average = **85s** productive testing
- Ratio: **95% wasted / 5% productive**

**After Level 1 fix (optimistic):**
- 10 files fixed, 2 still timeout × 120s = **240s (4 minutes)** wasted
- 27 healthy files × ~5s average = **135s** productive
- Ratio: **64% wasted / 36% productive** (7× improvement)

**After Level 2 fix (medium-term):**
- 0 timeouts
- 29 healthy files × ~3s average (slice modules) = **87s** total runtime
