# T1.12 — Fix E2E Timeouts: PARTIAL COMPLETION

**Date:** 2025-12-29  
**Status:** ⚠️ **BLOCKED** by seed script issue
...
- Added `createE2EApp()` helper that enforces proper lifecycle:
  ...
  4. **Call `app.enableShutdownHooks()`** ← CRITICAL
...
## REQUIRED NEXT STEPS (T1.12a — Separate Task)

### 1. Fix Seed Script ⚠️ **CRITICAL**
...
|---------|---------|--------|----------|
| `pnpm test:e2e:teardown-check` | 30s | ✅ PASS with warnings | ~1s |
| `node scripts/e2e-runtime-matrix.mjs` (baseline) | 30m | ⏱️ Budget exhausted | 25m 11s |
| `pnpm test:e2e -- test/a3-pos.e2e-spec.ts` (before fix) | 3m | ⏰ TIMEOUT | 3m |
| `pnpm lint` | 5m | ⚠️ 122 warnings, 0 errors | ~30s |
| `pnpm test:e2e -- test/blackbox/billing.blackbox.e2e-spec.ts` | 30s | ✅ PASS | ~2s |
| `node scripts/e2e-runtime-matrix.mjs` (after fix) | 30m | ⏱️ Budget exhausted, **still 12 timeouts** | 25m 2s |
| `pnpm test:e2e:setup` | 2m | ❌ FAIL (seed broken) | ~20s |
| `prisma migrate reset --force --skip-seed` | 2m | ✅ SUCCESS | ~45s |
| `pnpm test:e2e:setup` (after reset) | 2m | ❌ FAIL (jobRole column missing) | ~15s |
...
| **Timed out** | **12** | **12** | **0 improvement** ⚠️ |
...
✅ **NO RESETS NEEDED** — All changes are beneficial even though they didn't fix the timeout root cause:
...
**Lint status:** ✅ 0 errors, 122 warnings (unused imports — cosmetic)
