# T1.10 — Deterministic E2E Runner Completion Report
...
## STEP 0 — Current Behavior Verification (NO CODE CHANGES)
...
### Findings
- ✅ **Billing slice**: 11 tests passed, but Jest **did not exit cleanly** ("Jest did not exit one second after the test run has completed")
- ❌ **Franchise slice**: 9 tests failed (JWT module DI issue), and Jest **did not exit cleanly**
- ❌ **JSON output files** were **NOT created** (`.e2e-billing.json`, `.e2e-franchise.json` missing)
...
## STEP 1 — Hypotheses for Timeout Ambiguity
...
✅ **No changes needed** - script was already wired in package.json.
...
```
**Result:** ✅ **PASSED** (126 warnings, 0 errors)
...
**Result:** ✅ **DETERMINISTIC TIMEOUT** at exactly 25m 0s
...
### ✅ The wrapper provides deterministic states:
...
### ✅ The wrapper enforces hard deadlines:
...
### ✅ The wrapper ensures dataset clarity:
- Defaults to `DEMO_TAPAS` if `E2E_DEMO_DATASET` not set
- Logs dataset at start: `🗂️  Dataset: DEMO_TAPAS`
- Records dataset in status artifact

### ✅ The wrapper provides progress visibility:
- Heartbeat every 30s: `⏱️  Still running... Xm Ys elapsed`
...
✅ **T1.10 Goal Achieved:**
1. ✅ Runs Jest with a hard internal deadline (25m configurable)
2. ✅ Produces an unambiguous status artifact: `COMPLETED` vs `TIMED_OUT` vs `KILLED`
3. ✅ Ensures dataset visibility (default `DEMO_TAPAS`, logged and recorded)
4. ✅ Never leaves orphan processes (process group termination on Unix)
5. ✅ Provides progress visibility (heartbeat every 30s)
6. ✅ Saves partial output on timeout (`.raw` file for debugging)
